...
function BookingCard({ booking }: { booking: FlightBooking }) {
  ...
  const color = STATUS_COLORS[booking.status] ?? MUTED;
  ...
  return (
    ...
    >
      ...
      <View style={styles.airlineRow}>
        <View style={{ flexDirection: 'row-reverse', alignItems: 'center', gap: 10, flex: 1 }}>
          {offer.airlineLogoUrl ? (
            ...
            <View style={[styles.logo, { backgroundColor: DARK3, borderRadius: 8, alignItems: 'center', justifyContent: 'center' }]}>
      ...
      <View style={styles.routeRow}>
        <View style={{ alignItems: 'flex-end' }}>
          <Text style={styles.airportCode}>{offer.toAirport}</Text>
          <Text style={styles.sub}>{formatTime(arriveStr)}</Text>
        </View>
        <View style={styles.routeMiddle}>
          <View style={styles.routeLine} />
          <Ionicons name="airplane" size={16} color={GOLD} style={{ transform: [{ rotate: '180deg' }] }} />
          <View style={styles.routeLine} />
        </View>
        <View style={{ alignItems: 'flex-end' }}>
          <Text style={styles.airportCode}>{offer.fromAirport}</Text>
          <Text style={styles.sub}>{formatTime(departStr)}</Text>
        </View>
      </View>
      ...
      <View style={[styles.metaRow, { marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: BORDER }]}>
        <View style={{ alignItems: 'flex-start' }}>
          <Text style={styles.refLabel}>PNR</Text>
          <Text style={styles.refValue}>{booking.bookingReference ?? '—'}</Text>
        </View>
        <View style={{ alignItems: 'flex-end' }}>
          <Text style={styles.refLabel}>رقم الطلب</Text>
      ...
      <View style={styles.arrowWrap}>
        ...
        <Text style={styles.arrowText}>عرض التذكرة</Text>
      </View>
    </TouchableOpacity>
  );
}
    ...
    <View style={styles.screen}>
      ...
      >
        ...
        {!isLoading && !isError && (!bookings || bookings.length === 0) && (
          <View style={styles.center}>
            <Ionicons name="airplane-outline" size={60} color={MUTED} />
            <Text style={styles.centerText}>لا توجد رحلات محجوزة بعد</Text>
            <TouchableOpacity onPress={() => router.replace('/(tabs)' as any)} style={styles.retryBtn}>
              <Text style={{ color: GOLD, fontFamily: 'Tajawal_700Bold', fontSize: 14 }}>ابحث عن رحلة</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {!isLoading && bookings && bookings.length > 0 && (
          <>
            <Text style={styles.countText}>{bookings.length} رحلة</Text>
...
const styles = StyleSheet.create({
  ...
  routeLine: { flex: 1, height: 1, backgroundColor: BORDER },
  airportCode: { color: WHITE, fontFamily: 'Tajawal_800ExtraBold', fontSize: 20 },

  metaRow: { flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'center' },

  refLabel: { color: MUTED, fontFamily: 'Tajawal_400Regular', fontSize: 11 },
  refValue: { color: GOLD, fontFamily: 'Tajawal_800ExtraBold', fontSize: 13, letterSpacing: 0.5 },

  arrowWrap: { flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', marginTop: 10, gap: 4 },
  arrowText: { color: GOLD, fontFamily: 'Tajawal_700Bold', fontSize: 12 },

  center: { alignItems: 'center', paddingVertical: 60, gap: 14 },
  centerText: { color: MUTED, fontFamily: 'Tajawal_500Medium', fontSize: 15, textAlign: 'center' },
  retryBtn: {
    paddingHorizontal: 24, paddingVertical: 12,
    backgroundColor: 'rgba(201,160,96,0.12)', borderRadius: 14,
    borderWidth: 1, borderColor: 'rgba(201,160,96,0.3)',
  },
  countText: { color: MUTED, fontFamily: 'Tajawal_400Regular', fontSize: 13, textAlign: 'right', marginBottom: 12 },
});
