...
- [Mobile entry splash, no null](mobile-entry-splash.md) — entry screen must render branded splash (not `return null`) while auth loads or /api/auth/me hangs → else blank white screen; also: native Expo app can't be delivered via Replit Publish.
- [Production CORS allowlist](prod-cors-allowlist.md) — api-server CORS must trust REPLIT_DOMAINS (prod origins), not just dev domains; otherwise published admin dashboard login fails with 500/"connection error".
- [Prod DB writes / admin recovery](prod-db-write-recovery.md) — prod executeSql is read-only; to reset one prod row (e.g. admin password) without wiping data, use a secret-guarded recovery endpoint + republish (publish syncs schema, not data). Dashboard access needs role='admin', not just null permissions.
